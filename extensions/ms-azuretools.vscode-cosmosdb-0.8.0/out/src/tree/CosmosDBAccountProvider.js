...
class CosmosDBAccountProvider {
    ...
    loadMoreChildren(node) {
        return __awaiter(this, void 0, void 0, function* () {
            const client = getCosmosDBManagementClient_1.getCosmosDBManagementClient(node.credentials, node.subscriptionId);
            const accounts = yield client.databaseAccounts.list();
            let accountTreeItems = [];
            yield Promise.all(accounts.map((databaseAccount) => __awaiter(this, void 0, void 0, function* () {
                try {
                    let account = yield this.initChild(client, databaseAccount);
                    accountTreeItems.push(account);
                }
                catch (e) {
                    const err = vscode_azureextensionui_1.parseError(e);
                    //tslint:disable-next-line:no-non-null-assertion
                    accountTreeItems.push({ label: databaseAccount.name, description: "Invalid: " + err.message });
                }
            })));
