    ...
    createTreeItem(connectionString, api, label, id, isEmulator) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (api === experiences_1.API.MongoDB) {
                ...
                label = label || `${parsedId} (${experiences_1.getExperience(api).shortName})`;
                switch (api) {
                    case experiences_1.API.Table:
                        treeItem = new TableAccountTreeItem_1.TableAccountTreeItem(parsedId, label, endpoint, masterKey, isEmulator);
                        break;
                    case experiences_1.API.Graph:
                        treeItem = new GraphAccountTreeItem_1.GraphAccountTreeItem(parsedId, label, endpoint, undefined, masterKey, isEmulator);
                        break;
                    case experiences_1.API.DocumentDB:
                        treeItem = new DocDBAccountTreeItem_1.DocDBAccountTreeItem(parsedId, label, endpoint, masterKey, isEmulator);
                        break;
                    default:
