...
var VSLS_1 = require("../workspace/contract/VSLS");
exports.TraceSources = VSLS_1.TraceSources;
exports.TraceEventIds = VSLS_1.TraceEventIds;
exports.traceSource = new TraceSource('Client');
class GlobPatternTraceFilter {
    constructor(globToVerbosity) {
        this.globToVerbosity = globToVerbosity;
        this.traceSourceVerbosityCache = {};
    }
    getTraceVerbosity(source) {
        let traceVerbosity = this.traceSourceVerbosityCache[source];
        // If we don't have a cached verbosity for this source, find the verbosity and cache it
        if (typeof traceVerbosity === 'undefined') {
            traceVerbosity = Number.MAX_VALUE;
            if (this.globToVerbosity) {
                for (let traceGlob in this.globToVerbosity) {
                    if (minimatch(source, traceGlob)) {
                        traceVerbosity = TraceEventType[this.globToVerbosity[traceGlob]];
                    }
                }
            }
            this.traceSourceVerbosityCache[source] = traceVerbosity;
        }
        return traceVerbosity;
    }
    shouldTrace(source, eventType, id) {
        return this.getTraceVerbosity(source) >= eventType;
    }
}
