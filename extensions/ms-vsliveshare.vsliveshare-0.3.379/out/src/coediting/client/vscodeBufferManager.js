...
class VSCodeBufferManager {
    ...
    onDidChangeTextEditorVisibleRanges(fileName, ranges) {
        if (session_1.SessionContext.EnableVerticalScrolling && ranges.length > 0) {
            const firstVisibleLine = ranges[0];
            const bufferOffsetStart = this.toCollabOffset(firstVisibleLine.start);
            const bufferOffsetEnd = this.toCollabOffset(firstVisibleLine.end);
