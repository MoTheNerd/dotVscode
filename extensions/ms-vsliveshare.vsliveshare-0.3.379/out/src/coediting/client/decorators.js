...
class ClientDecoratorManager {
    ...
    dispose() {
        ...
        SharedColors.releaseColor(this.clientId);
    }
    updateDecorators() {
...
class SharedColors {
    static requestColor(clientId) {
        ...
        if (SharedColors.assignedColors[clientId]) {
            return SharedColors.assignedColors[clientId];
        }
        let colorIndex;
        if (SharedColors.availableColors.length) {
            const color = SharedColors.availableColors.pop();
            SharedColors.assignedColors[clientId] = color;
            return color;
        }
        // TODO More graceful handling of too many collaborators for number of available colors.
        // For now just return a duplicate random color without registering the client to it.
        colorIndex = Math.floor(Math.random() * SharedColors.allColors.length);
        return SharedColors.allColors[colorIndex];
    }
    static releaseColor(clientId) {
        SharedColors._initcolors();
        if (SharedColors.assignedColors[clientId]) {
            const color = SharedColors.assignedColors[clientId];
            SharedColors.availableColors.push(color); // Colors might get out of priority order if multiple people leave and rejoin, but it's not a big deal. Initial colors will always be assigned in order or priority.
            delete SharedColors.assignedColors[clientId];
        }
    }
    static _initcolors() {
        if (SharedColors.allColors && SharedColors.availableColors && SharedColors.assignedColors) {
            return;
        }
        ...
        SharedColors.availableColors = SharedColors.allColors.slice();
        SharedColors.assignedColors = {};
    }
