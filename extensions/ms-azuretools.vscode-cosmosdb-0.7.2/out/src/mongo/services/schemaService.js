...
class SchemaService {
    ...
    queryCollectionSchema(collectionName) {
        return 'mongo://query/' + collectionName + '.schema';
    }
    aggregateCollectionSchema(collectionName) {
        return 'mongo://aggregate/' + collectionName + '.schema';
    }
    queryDocumentUri(collectionName) {
        return 'mongo://query/' + collectionName + '.json';
    }
    aggregateDocumentUri(collectionName) {
    ...
    _resolveQueryCollectionSchema(collectionName, schemaUri) {
    ...
    _resolveAggregateCollectionSchema(collectionName) {
        const collection = this._db.collection(collectionName);
        const cursor = collection.find();
        return new Promise((resolve, _reject) => {
    ...
    setSchemaForDocument(parent, document, schema) {
    ...
    setSchemaForDocumentProperty(parent, property, document, schema) {
    ...
    setGlobalOperatorProperties(schema) {
...
Use the $where operator to pass either a string containing a JavaScript expression or a full JavaScript function to the query system`
    ...
    setLogicalOperatorProperties(schema, schemaUri) {
    ...
    setOperatorProperties(type, schema) {
        ...
        schema.properties = Object.assign({}, expressionSchema.properties);
        schema.properties.$not = {
            ...
            properties: Object.assign({}, expressionSchema.properties)
        };
        schema.properties.$elemMatch = {
            type: 'object',
        };
    }
    getAggregateStagePropertiesSchema(querySchemaUri) {
        const schemas = [];
        schemas.push({
            type: 'object',
            properties: {
    ...
    readNext(result, cursor, batchSize, callback) {
